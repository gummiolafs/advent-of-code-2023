...
public static class Day3GearRatiosData
{
    public static Day3GearRatiosModels.EngineSchematic GetSchematic(string inputData)
    {
        var lines = inputData.Split(Environment.NewLine, StringSplitOptions.TrimEntries);

        var results = lines.Select((l, y) =>
        {
            var numbers = new List<Day3GearRatiosModels.Number>();
            int xStart = -1;
            int value = 0;
            ...
            for (var i = 0; i < l.Length; i++)
            {
                ...
                {
                    value = (value * 10) + (l[i] - '0');
                    xStart = xStart == -1 ? i : xStart;
                }
                else if (xStart != -1)
                {
                    numbers.Add(new Day3GearRatiosModels.Number(value, xStart, i, y));
                    xStart = -1;
                    value = 0;
                }
            }
            ...
            if (xStart != -1)
                numbers.Add(new Day3GearRatiosModels.Number(value, xStart, l.Length-1, y));

            var symbols = l.Select((c, x) => (c, x))
                .Where(c => !char.IsNumber(c.Item1) && c.Item1 != '.')
                .Select(c => (c.Item1, c.Item2, y));

            return (numbers, symbols);
        }).ToArray();

        return new Day3GearRatiosModels.EngineSchematic(results.SelectMany(r => r.numbers),
            results.SelectMany(r => r.symbols));
    }
