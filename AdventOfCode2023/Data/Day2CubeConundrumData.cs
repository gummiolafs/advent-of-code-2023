...
public static class Day2CubeConundrumData
{
    public static IEnumerable<Day2CubeConundrumModels.Game> GetGames(string inputData)
    {
        var lines = inputData.Split(Environment.NewLine, StringSplitOptions.TrimEntries);
        
        var games = new List<Day2CubeConundrumModels.Game>();
        foreach (var line in lines)
        {
            ...
            var gameNumber = int.Parse(gameSplit[0][5..]);

            var draws = new List<Day2CubeConundrumModels.Cubes>();
            
            foreach (var drawSplit in gameSplit[1].Trim().Split(';'))
            {
                var red = 0;
                var green = 0;
                var blue = 0;
                
                foreach (var colorSplit in drawSplit.Trim().Split(','))
                {
                    var littleSplit = colorSplit.Trim().Split(' ');

                    switch (littleSplit[1])
                    {
                        case "red":
                            red = int.Parse(littleSplit[0]);
                            break;
                        case "green":
                            green = int.Parse(littleSplit[0]);
                            break;
                        case "blue":
                            blue = int.Parse(littleSplit[0]);
                            break;
                    }
                }
                
                draws.Add(new Day2CubeConundrumModels.Cubes(red, green, blue));
            }
            games.Add(new Day2CubeConundrumModels.Game(gameNumber, draws));
        }

        return games;
    }

    public const string InputData =
        """
